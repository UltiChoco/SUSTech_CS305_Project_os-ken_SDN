...
实验环境：Mininet 2.3.0 + iperf + tcpdump，数据分析基于 Python/matplotlib。
...
|---|---|---|
| 平均吞吐量 | **10.44 Mbps** | **9.56 Mbps** |
| 最大吞吐量 | 49.30 Mbps | 16.80 Mbps |
| 最小吞吐量 | 0.25 Mbps | 0.92 Mbps |
| 标准差 | 7.31 Mbps | 2.17 Mbps |
| 瓶颈利用率 | ~104% * | ~96% |

> \* Reno 平均超 10Mbps 是因为慢启动阶段填满瓶颈缓冲时短暂吞吐量暴增（burst），排除首秒后稳定在 9.4 Mbps 左右。
...
- 在单流、10Mbps/20ms 瓶颈下，Reno (10.44 Mbps) 与 Cubic (9.56 Mbps) 吞吐量相近，雷诺初始 burst 后也稳定在 9.4 Mbps 左右
- Cubic 吞吐量更稳定（标准差 2.17 vs 7.31），波动更小
...
- Bufferbloat 是真实且严重的问题：大缓冲可使延迟恶化 **16 倍**（20ms → 322ms）
...
1. **TCP 拥塞控制算法对比**——在 10Mbps/20ms 单流场景下，Reno (10.44 Mbps) 与 Cubic (9.56 Mbps) 均能饱和瓶颈，Cubic 吞吐量更稳定（标准差 2.17 vs 7.31）
2. **Bufferbloat 是延迟杀手**——200 包缓冲使平均 RTT 从 28ms 飙升至 178ms（**6.3 倍**），峰值达 251ms，以延迟为代价换取了零丢包
